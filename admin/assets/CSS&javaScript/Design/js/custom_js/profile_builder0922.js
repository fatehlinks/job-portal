function edit_work(a) {
    ...
    var url = $("#baseImageUrl").val();
    var urledu = $("#baseImageUrlEdu").val();
    var urlexp = $("#baseImageUrlExp").val();
    $('#w_att_' + b + '').remove();
    if (j != "" && j != null) {
        $('#w_att_' + b + '').remove();
        ...
        $('#old_attach_exp_' + b + '').append('<img id="old_attach_exp_img_' + b + '" src="' + urlexp + j + '" style="width:30px;"><br/><span style="font-size:10px;">Scanned Document Max:2MB (Only jpg,jpeg,png &amp; pdf allowed)</span>');
    } else {
        $('#ed_work_attach_document').next().remove();
    }
    ...
    if (old_attach != '') {
        $('#old_attach').append('<img id="old_attach_img" src="' + urlexp + old_attach + '" height="50" alt="Image">');
    } else {
...
function edit_academics(a) {
    var b = a, c = $("#edit_degree_level" + b).val(),
        d = $("#edit_level_name" + b).val(),
        e = $("#edit_degree_name" + b).val(),
        f = $("#edit_degree_institute" + b).val(),
        academic_institute = $("#edit_academic_institute" + b).val(),
        g = $("#edit_degree_major" + b).val(),
        h = $("#edit_degree_start_date" + b).val(),
        i = $("#edit_degree_end_date" + b).val(),
        j = $("#edit_degree_country" + b).val(),
        k = $("#edit_degree_percentage" + b).val(),
        l = $("#edit_position_holder" + b).val(),
        m = $("#edit_degree_attachment" + b).val();
    ...
    var degree_verification_number = $("#edit_degree_verification_number" + b).val();
    console.log('degree_verification_number', degree_verification_number);
    $('#s_att_' + b + '').remove();
    ...
    $("#ed_degree_verification_number").val(degree_verification_number);
    console.log('degree_verification_number', degree_verification_number);
    if (m != "" && m != null) {
        $('#s_att_' + b + '').remove();
        ...
        $('#old_attach_edu_' + b + '').append('<img id="old_attach_edu_img_' + b + '" src="' + urledu + m + '" height="50"><br/><span style="font-size:10px;">Scanned Document Max:2MB (Only jpg,jpeg,png &amp; pdf allowed)</span>');
    } else {
        $('#ed_attach_document').next().remove();
    }
    ...
    if (c == 'D11' || c == 'D12') {
        $('#ed_marks_per').prop('disabled', true);
        $('#ed_degree_position').prop('disabled', true);
        $('#ed_degree_is_cgpa').prop('disabled', true);
    }
}
...
function edit_ach(a) {
    var b = a, c = $("#edit_achievement_title" + b).val(), d = $("#edit_achievement_des" + b).val();
    $("#ed_achievement_title").val(c), $("#ed_achievement_descriptions").val(d), $("#ach_id").val(b), $("#old_achievement_name").val(c), $("#old_achievement_descriptions").val(d), $("#edit-achievement").modal("show")
}
function del_ach(a) {
    var b = a;
    $("#delete_achievement").modal("show"), $("#yes_delete_achievement").click(function () {
        var a = $("#url").attr("value");
        $("#loading_modal").modal("show"), setTimeout(function () {
            $.ajax({ url: a + "new_profile_builder/delete_achievement/" + b }), $("#loading_modal").modal("hide"), $("#achievement_table #" + b).hide(), $("#delete_achievement").modal("hide"), $(".notification").html("<div class='alert success_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Success! </strong>  Record has been deleted!</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)
        }, 1e3)
    })
}
...
function check_married() {
    ...
    "Female" == a && "Married" == b ? $("#husband_name").prop("disabled", !1) : ($("#husband_name").val(""),
        $("#information_profile").bootstrapValidator("resetField", "husband_name"),
        $("#husband_name").prop("disabled", !0))



    if ("Female" == a && "Married" == b) {
        console.log('a', a);
        console.log('b', b);
        $("#husband_req").show();
    } else {
...
function getDegreebyLevel(a, b, id) {
    ...
    $.ajax({
        url: d, data: { degree_level: a }, type: "POST", success: function (a) {
            var a = $.parseJSON(a);
            if ("success" == a.status) {
                $("#" + b).html("");
...
function adjustModalScroll(a, b, c, d, e) {
    "shown.bs.modal" == b ? $("#" + a).on(b, function () {
        ...
        null == c && "" == c || ($(".add_degree_select").select2("val", ""),
         document.getElementById(c).reset(),
          $("#" + c).bootstrapValidator("resetForm", !0),
           $("#" + c).bootstrapValidator("resetField", "degree_start_date"),
            $("#" + c).bootstrapValidator("resetField", "degree_end_date"),
             $(".add_degree_select").select2("val", ""), 
             $(".add_ref_select").select2("val", ""),
              $(".ed_ref_select").select2("val", ""),
               (d.length > 0 || null != d) && $(d).each(function (a, b) {
...
function restrictFileUpload(inputElement) {

    console.log('inputElement', inputElement);
    const file = inputElement.files[0];
    const fileSizeLimit = 5 * 1024 * 1024; // 5 MB in bytes
    const allowedFileTypes = /(\.jpg|\.jpeg|\.png|\.pdf)$/i; // Regular expression for file types (images and PDFs)
    const maxFilesAllowed = 1; // Number of files allowed to upload
    const errorSpan = document.getElementById('fileError');

    console.log(file);
    console.log(fileSizeLimit);
    console.log(allowedFileTypes);
    console.log(maxFilesAllowed);
    console.log(errorSpan);
    // Reset error message
    errorSpan.innerText = '';

    // Check Number of Files
    if (inputElement.files.length > maxFilesAllowed) {
        errorSpan.innerText = `Only ${maxFilesAllowed} file is allowed to upload.`;
    ...
    if (!allowedFileTypes.exec(file.name)) {
        errorSpan.innerText = 'Only.jpg,.jpeg,.png, and.pdf files are allowed.';
        inputElement.value = ''; // Reset input
        return;
    }
    ...
    if (file.size > fileSizeLimit) {
        errorSpan.innerText = `File is too large. Maximum size allowed is ${fileSizeLimit / (1024 * 1024)} MB.`;
        inputElement.value = ''; // Reset input
        return;
    }

    // If all checks pass, you can proceed with the upload or other actions
    console.log('File is valid. Proceeding with upload...');
    // Uncomment the line below to see the file details in the console
    // console.log(file);
}
...
$(document).ready(function () {
    $("#cnic_token").css({ display: "none" }), $("#cnic_expire").css({ display: "none" }), $(".notification").delay(3200).fadeOut(300), tinymce.init({
        selector: "#job_responsibilities", height: 250, browser_spellcheck: !0, max_word: 2, theme: "modern", statusbar: !1, plugins: ["advlist autolink lists link charmap print preview anchor", "searchreplace visualblocks code fullscreen autoresize", "insertdatetime table contextmenu paste code"], content_css: [$("#base_url").val() + "assets/recruitment/tinymce/codepen.min.css"], setup: function (a) {
        ...
        selector: "#ed_job_responsibilities", invalid_elements: 'iframe,script', height: 250, browser_spellcheck: !0, max_word: 2, theme: "modern", statusbar: !1, plugins: ["advlist autolink lists link charmap print preview anchor", "searchreplace visualblocks code fullscreen autoresize", "insertdatetime table contextmenu paste code"], content_css: [$("#base_url").val() + "assets/recruitment/tinymce/codepen.min.css"], setup: function (a) {
            a.on("keyup", function (a) {
                var b = 5e3, c = tinyMCE.activeEditor.getContent({ format: "text" });
    ...
    1 == a && ($("#cnic_token").css({ display: "block" }), $("#txt_cnic_token").prop("disabled", !1), $("#expected_date").css({ display: "block" }), $("#txt_expected_date").prop("disabled", !1), $("#cnic_expire").css({ display: "none" }), $("#cnic_front_attachment_div").css({ display: "none" }), $("#cnic_back_attachment_div").css({ display: "none" }), $("#txt_cnic_expire").prop("disabled", !0), $("#txt_cnic_expire").val("")), 0 == a && ($("#cnic_expire").css({ display: "block" }), $("#txt_cnic_expire").prop("disabled", !1), $("#cnic_token").css({ display: "none" }), $("#txt_cnic_token").prop("disabled", !0), $("#txt_cnic_token").val(""), $("#expected_date").css({ display: "none" }), $("#txt_expected_date").prop("disabled", !0), $("#txt_expected_date").val("")), "" == a && ($("#cnic_token").css({ display: "none" }), $("#txt_cnic_token").prop("disabled", !0), $("#txt_cnic_token").val(""), $("#expected_date").css({ display: "none" }), $("#txt_expected_date").prop("disabled", !0), $("#txt_expected_date").val(""), $("#cnic_expire").css({ display: "none" }), $("#txt_cnic_expire").prop("disabled", !0), $("#txt_cnic_expire").val("")),
        ...
        $("#expiry_option__").on("change", function (a) {
            a.preventDefault();
    ...
    $("#expiry_option").on('change', function (e) {
        e.preventDefault();
        var cnic_option = $("#expiry_option").val();
        if (cnic_option == 1) {
            $("#cnic_token").css({ 'display': 'block' });
            $("#txt_cnic_token").prop('disabled', false);
            $("#expected_date").css({ 'display': 'block' });
            $("#txt_expected_date").prop('disabled', false);

            $("#cnic_expire").css({ 'display': 'none' });
            $("#cnic_front_attachment_div").css({ 'display': 'none' });
            $("#cnic_back_attachment_div").css({ 'display': 'none' });

            $("#txt_cnic_expire").prop('disabled', true);
            $("#txt_cnic_expire").val('');

        }
        if (cnic_option == 0 || cnic_option == '2') {
            $("#cnic_expire").css({ 'display': 'block' });
            $("#txt_cnic_expire").prop('disabled', false);

            $("#cnic_front_attachment_div").css({ 'display': 'block' });
            $("#cnic_back_attachment_div").css({ 'display': 'block' });

            $("#cnic_token").css({ 'display': 'none' });
            $("#txt_cnic_token").prop('disabled', true);
            $("#txt_cnic_token").val('');
            $("#expected_date").css({ 'display': 'none' });
            $("#txt_expected_date").prop('disabled', true);
            $("#txt_expected_date").val('');
        }
        if (cnic_option == '' || cnic_option == '2') {
            $("#cnic_token").css({ 'display': 'none' });
            $("#txt_cnic_token").prop('disabled', true);
            $("#txt_cnic_token").val('');
            $("#expected_date").css({ 'display': 'none' });
    ...
    $("#cnic_id").mask("99999-9999999-9"), 
    // $("#mobile_num").mask("+99-9999-9999?99999"),
    $("#mobile_num").mask("0399-9999999"),
     $("#land_line").mask("+99-99999999?99999"),
      $("#ref_phone").mask("+99-99999999?99999"), $("#ed_ref_phone").mask("+99-99999999?99999"), $("#txt_cnic_token").mask("999999999999"),
        $("#information_profile").bootstrapValidator({
            excluded: [":disabled"],
            feedbackIcons: {
                ...
                return $.ajax({
                    url: d + "new_profile_builder/information", type: "POST", data: form, cache: !1, contentType: !1, processData: !1, mimeType: "multipart/form-data", success: function (a) {
                        setTimeout(function () {
                            $(".pre-loader2").hide();
                            var b = $.parseJSON(a);
                            //var b = a;
                        ...
                        $("#loading_modal").modal('hide');
                        var errorMessage = `An unexpected error occurred. Please try again later. (Status: ${a.status} ${a.statusText})`;
                        var notificationHtml = `<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong> Failure! </strong> ${errorMessage}</div>`;
                        $(".notification").html(notificationHtml).fadeIn(300).delay(6000).fadeOut(300); // Increased duration to 6 seconds
                        // Optional: Log the error (e.g., using a client-side error logging service)
            ...
            fields: {
                ...
                father_name: {
                    validators: {
                        ...
                        stringLength: {
                            min: 3,
                            max: 100,
                            message: 'The father name must be greater than 3 and less than 100 characters'
                        },
                        regexp: {
                            regexp: /^[a-z .\s]+$/i,
                            message: "Please enter alphabets only"
                ...
                husband_name: {
                    validators: {
                        ...
                        stringLength: {
                            min: 3,
                            max: 100,
                            message: 'The husband name must be greater than 3 and less than 100 characters'
                        },
                        regexp: {
                            regexp: /^[a-z\s]+$/i,
                            message: "Please enter alphabets only"
                ...
                gender: {
                    validators: {
                        ...
                        stringLength: {
                            min: 4,
                            max: 20,
                            message: 'The gender field must be greater than 4 and less than 10 characters'
                        }
                    }
                },
                age: {
                ...
                marital_status: {
                    validators: {
                        notEmpty: {
                            message: "Please marital status"
                        },
                        stringLength: {
                            min: 4,
                            max: 15,
                            message: 'The marital status must be greater than 4 and less than 15 characters'
                        }
                    }
                },
                expiry_option: {
                    validators: {
                        notEmpty: {
                            message: "Please select cnic expiry option"
                        }
                    }
                },
                cnic_expiry_date: {
                    validators: {
                        callback: {
                            message: "Please enter the CNIC expiry date.",
                            callback: function (value, validator, $field) {
                                var expiryOption = $('#expiry_option').val();
                                // Require CNIC expiry date only if expiryOption is 0
                                return (expiryOption === '0' && value !== '') || expiryOption !== '0';
                            }
                        }
                    }
                },
                cnic_token: {
                    validators: {
                        callback: {
                            message: "Please enter the CNIC token.",
                            callback: function (value, validator, $field) {
                                var expiryOption = $('#expiry_option').val();
                                // console.log(expiryOption)
                                // console.log(value)
                                // Require CNIC token only if expiryOption is 1
                                return (expiryOption === '1' && value !== '') || expiryOption !== '1';
                            }
                        }
                    }
                },
                expected_date: {
                    validators: {
                        callback: {
                            message: "Please enter the CNIC expected date.",
                            callback: function (value, validator, $field) {
                                var expiryOption = $('#expiry_option').val();
                                // Require expected date only if expiryOption is 1
                                return (expiryOption === '1' && value !== '') || expiryOption !== '1';
                            }
                        }
                    }
                ...
                residential_address: {
                    validators: {
                        ...
                        stringLength: {
                            min: 5,
                            max: 500,
                            message: 'The residential address must be greater than 3 and less than 100 characters'
                        },
                        regexp: {
                            regexp: /^[a-zA-Z0-9\/#&\d\-_.(),\s]+$/i,
                ...
                religion: {
                    validators: {
                        ...
                        stringLength: {
                            min: 3,
                            max: 100,
                            message: 'The religion must be greater than 3 and less than 100 characters'
                        },
                        regexp: {
                            regexp: /^[a-z\s]+$/i,
                            message: "Please enter alphabets only"
                ...
                objective: {
                    validators: {
                        stringLength: {
                            min: 10,
                            max: 2000,
                            message: 'The objective must be greater than 10 and less than 2000 characters'
                        },
                        regexp: {
                            regexp: /^[a-zA-Z0-9\d\-_.()!+=#&,\s]+$/i,
                ...
                is_gov_off: {
                    validators: {
                        between: {
                            min: 0,
                            max: 1,
                            message: "Only select YES or NO"
                        },
                        regexp: {
                            regexp: /^[0-9\s]+$/i,
                            message: "Please enter numerics only"
                        }
                    }
                },
                is_disabled: {
                ...
                cand_noc: {
                    validators: {
                        file: {
                            ...
                            maxSize: 2097152,
                            message: "The selected file is not valid only (jpeg,jpg,png,pdf) formats & max 2MB size allowed"
                        }
                    }
                },
                domicile_front_attachment: {
                    validators: {
                        file: {
                            extension: "jpeg,jpg,png,pdf",
                            type: "image/jpeg,image/png,application/pdf",
                ...
                police_character_certificate_attachment: {
                    validators: {
                        file: {
                            extension: "jpeg,jpg,png,pdf",
                            type: "image/jpeg,image/png,application/pdf",
        ...
        $("#target_job_forms").bootstrapValidator({
            excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
                var d = $("#url").val(), e = $("#candidate_id").val(), f = $("#candidate_cnic").val();
                return $("#loading_modal").modal("show"), $.ajax({
                    url: d + "new_profile_builder/target_job", data: $("#target_job_forms").serialize() + "&candidate_id=" + e + "&candidate_cnic=" + f, type: "POST", success: function (a) {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        $("#add_employee_job_information").bootstrapValidator({
            excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
                $(".pre-loader2").show();
                tinyMCE.triggerSave();
                var d = $("#url").val(),
                ...
                return $.ajax({
                    url: k, type: "POST", data: l, cache: !1, contentType: !1, processData: !1, mimeType: "multipart/form-data", success: function (a) {
                        //setTimeout(function () {
                        ...
                        if ("data_error" == b.status) {
                            // $("#loading_modal").modal("hide"), $("#add-emp-job-information").modal("hide"),
                            // $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong> b.errors </div>").fadeIn(300),
                            ...
                            $("#loading_modal").modal("hide");
                            $("#add-emp-job-information").modal("hide");
                            $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a> <strong> Failure! </strong> " + b.errors + "</div>").fadeIn(300);
                            $(".notification").delay(3200).fadeOut(300);
                ...
                }, add_work_attach_document: { validators: { notEmpty: { message: "Attachement is required" }, file: { extension: "jpeg,jpg,png,pdf", type: "image/jpeg,image/png,application/pdf", maxSize: 2097152, message: "The selected file is not valid only (jpeg,jpg,png,pdf) formats & max 2MB size allowed" } } }
            }
        }), $("#edit_employee_job_information").bootstrapValidator({
            excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
                if ($('#ed_present').val() == 0) {
                    $('#ed_desig_end_date').prop('disabled', false);
                    $('#ed_desig_end_date').removeAttr('readonly');
                }
                // # if h empty then set present
                // if ($('#ed_desig_end_date').val() == '') {
                //     $('#ed_present').val(1);
                // }
                var d = $("#edit_work_id").val(),
                 e = $("#url").val(),
                  f = $("#ed_designation").val(),
                   g = $("#ed_desig_start_date").val(),
                    h = $("#ed_desig_end_date").val(),
                     i = $("#ed_company_name").val(),
                      j = $("#ed_job_level").find("option:selected").text(),
                       job_level_id = $("#ed_job_level").val(),
                        k = tinyMCE.get("ed_job_responsibilities").getContent({ format: "html" });
                ...
                return m.append("ed_job_responsibilities", k), tinyMCE.triggerSave(), $.ajax({
                    url: l, type: "POST", data: m, cache: !1, contentType: !1, processData: !1, mimeType: "multipart/form-data", success: function (a) {
                        setTimeout(function () {
                            var b = $.parseJSON(a);
                            if ("validation" == b.status)
                                ...
                                }), $("#edit-emp-job-information").css("overflow-x", "auto"), $("#edit-emp-job-information").css("overflow-y", "visible"),
                                 $("#edit-emp-job-information").animate({ scrollTop: 0 }, 600),
                                 $("#loading_modal").modal("hide"), !1;


                            // if ("validation" == b.status){

                            //     console.log('validation',b.errors);
                            //     $("#loading_modal").modal("hide"), 
                            //     $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong>  Error occured!</div>").fadeIn(300),
                            //      $(".notification").delay(3200).fadeOut(300)
                            //      return false;
                ...
                }, ed_work_attach_document: { validators: { file: { extension: "jpeg,jpg,png,pdf", type: "image/jpeg,image/png,application/pdf", maxSize: 2097152, message: "The selected file is not valid only (jpeg,jpg,png,pdf)formats & max 2MB size allowed" } } }
            }
        }), $('#academic_institute').change(function () {
    ...
    $("#add_degree_information_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            if ("Literate" == g || "Primary" == g || "Middle" == g) {
                ...
                "" == r && (r = "No");
                var dvnadd = $("#degree_verification_number").val();
            }
            var s = d + "new_profile_builder/degree_information";
            var t = b[0];
            var t = new FormData(t);
            return $.ajax({
                url: s, type: "POST", data: t, cache: !1, contentType: !1, processData: !1, mimeType: "multipart/form-data", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
            ...
            degree_institute: { validators: { regexp: { regexp: /^[a-zA-z _&)(-\s]+$/i, message: "Please enter alphabets only" } } }, academic_institute: { validators: { notEmpty: { message: "Please Select Institute" }, numeric: { message: 'Select From the given List', } } }, degree_level: { validators: { notEmpty: { message: "Degree Level is required" } } }, degree_name: { validators: { notEmpty: { message: "Please enter degree name" } } }, degree_major: { validators: { notEmpty: { message: "Please enter degree major" }, regexp: { regexp: /^[a-zA-z ,._&)(-\s]+$/i, message: "Please enter alphabets only" } } }, degree_start_date: { validators: { notEmpty: { message: "Please enter start date" } } }, degree_end_date: { validators: { notEmpty: { message: "Please enter end date" } } }, degree_country: { validators: { notEmpty: { message: "Please enter country" } } }, marks_per: { validators: { notEmpty: { message: "Percentage is required" }, between: { min: 33, max: 100, message: "Percentage must be between 33 to 100" }, regexp: { regexp: /^\d+(\.\d{1,2})?$/, message: "Decimal value upto two point" } } }, attach_document: { validators: { notEmpty: { message: "Attachement is required" }, file: { extension: "jpeg,jpg,png,pdf", type: "image/jpg,image/jpeg,image/png,application/pdf", maxSize: 2097152, message: "The selected file is not valid only (jpeg,jpg,png,pdf) formats & max 2MB size allowed" } } }, degree_is_cgpa: { validators: { numeric: { message: 'Select From the given List' } } }, degree_obtained_cgpa: { validators: { between: { min: 1, max: 5, message: "CGPA must be between 1 to 5" }, regexp: { regexp: /^[0-9.]+$/, message: "Decimal value upto two point" } } }, degree_total_marks: {
                ...
                validators: {
                    digits: { message: 'Only Digits Allowed' }, between: { min: 1, max: 10000, message: "Obtained marks be between 1 to 10000" }, callback: {
                        message: 'Obtained marks should be less than or equal to total marks', callback: function (value, validator) {
                            var ob_marks = $('#degree_obtained_marks').val();
                            var t_marks = $("#degree_total_marks").val();
                            if (t_marks === '') {
                                return false;
    ...
    }), $("#edit_degree_information_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            $(".pre-loader2").show();
            ...
            if ("D13" == f || "D1" == f || "D2" == f) {
                ...
                var h = $("#f_degree_level_edit").find("option:selected").text(),
                    k = $("#ed_degree_name").find("option:selected").text(),
                    i = $("#ed_degree_start_date").val(),
                    j = $("#ed_degree_end_date").val(),
                    r = $("#ed_degree_name").find("option:selected").val(),
                    e = r.split("_"),
                    l = $("#f_degree_level_edit").find("option:selected").val(),
                    m = $("#ed_degree_major").val(),
                    n = $("#ed_degree_country").find("option:selected").text(),
                    o = $("#ed_degree_country").find("option:selected").val(),
                    p = $("#ed_marks_per").val(),

                    q = $("#ed_degree_position").find("option:selected").val(),
                    dvn = $("#ed_degree_verification_number").val()
                    ;
                "" == q && (q = "No")
            }
            ...
            return $.ajax({
                url: s, type: "POST", data: t, cache: !1, contentType: !1, processData: !1, mimeType: "multipart/form-data", success: function (a) {
                    setTimeout(function () {
                        ...
                        $(".pre-loader2").hide();
                        var c = b.data_id;
                        (b.status = "success") ? ($("#edit-degree-information").modal("hide"),
                            $("#edurec_" + c).html(""), $("#edurec_" + c).append("<td>"
                                + g + "</td><td>"
                                + h + "</td><td>"
                                + k + "</td><td>"
                                + m + "</td><td>"
                                ...
                                + q + "</td><td>"
                                + dvn +
                                '</td><td><input type="hidden" value="' + g + '" id=edit_degree_institute' + c +
                                '><input type="hidden" value="' + academic_institute + '" id=edit_academic_institute' + c +
                                '><input type="hidden" value="' + l + '" id=edit_degree_level' + c +
                                '><input type="hidden" value="' + h + '" id=edit_level_name' + c +
                                '><input type="hidden" value="' + e[1] + '" id="edit_degree_name' + c +
                                '"><input type="hidden" value="' + m + '" id=edit_degree_major' + c +
                                '><input type="hidden" value="' + i + '" id=edit_degree_start_date' + c +
                                '><input type="hidden" value="' + j + '" id=edit_degree_end_date' + c +
                                '><input type="hidden" value="' + o + '" id=edit_degree_country' + c +
                                '><input type="hidden" value="' + p + '" id=edit_degree_percentage' + c +
                                '><input type="hidden" value="' + $("#ed_degree_position").find("option:selected").val() + '" id=edit_position_holder' + c +
                                '><input type="hidden" value="' + dvn + '" id=edit_degree_verification_number' + c +
                                '><input type="hidden" value="' + b.upload_image + '" id=edit_degree_attachment' + c +
                                '><input type="hidden" value="' + degree_is_cgpa + '" id=edit_degree_is_cgpa' + c +
                                '><input type="hidden" value="' + degree_total_marks + '" id=edit_degree_total_marks' + c +
                                '><input type="hidden" value="' + degree_obtained_marks + '" id=edit_degree_obtained_marks' + c +
                                '><input type="hidden" value="' + degree_obtained_cgpa + '" id=edit_degree_obtained_cgpa' + c +
                                '><a href="javascript:void(0)" onclick="edit_academics(' + c + ')" class="education_delete btn btn-success btn-sm"  data-tooltip="tooltip" title="Edit"><i class="fa fa-edit" aria-hidden="true"></i> </a> <a href="javascript:void(0)" onclick="delete_academics('
                                + c + ')" class="btn btn-primary btn-sm" data-tooltip="tooltip" title="delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>'),
            ...
            ed_degree_institute: { validators: { regexp: { regexp: /^[a-zA-z _&)(-\s]+$/i, message: "Please enter alphabets only" } } }, ed_academic_institute: { validators: { notEmpty: { message: "Please Select Institute" }, numeric: { message: 'Select From the given List', } } }, ed_degree_level: { validators: { notEmpty: { message: "Degree Level is required" } } }, ed_degree_name: { validators: { notEmpty: { message: "Please enter degree name" } } }, ed_degree_major: { validators: { notEmpty: { message: "Please enter degree major" }, regexp: { regexp: /^[a-z ,._&)(-\s]+$/i, message: "Please enter alphabets only" } } }, ed_degree_start_date: { validators: { notEmpty: { message: "Please enter start date" } } }, ed_degree_end_date: { validators: { notEmpty: { message: "Please enter end date" } } }, ed_degree_country: { validators: { notEmpty: { message: "Please enter country" } } }, ed_marks_per: { validators: { notEmpty: { message: "Percentage is required" }, between: { min: 33, max: 100, message: "Percentage must be between 33 to 100" }, regexp: { regexp: /^\d+(\.\d{1,2})?$/, message: "Decimal value upto two point" } } }, ed_attach_document: { validators: { file: { extension: "jpeg,jpg,png,pdf", type: "image/jpeg,image/png,application/pdf", maxSize: 2097152, message: "The selected file is not valid only (jpeg,jpg,png,pdf) formats & max 2MB size allowed" } } }, ed_degree_obtained_cgpa: { validators: { between: { min: 1, max: 5, message: "CGPA must be between 1 to 5" }, regexp: { regexp: /^\d+(\.\d{1,2})?$/, message: "Decimal value upto two point" } } }, ed_degree_total_marks: {
                validators: {
                    digits: { message: 'Only Digits Allowed' }, between: { min: 1, max: 10000, message: "CGPA must be between 1 to 10000" }, callback: {
                        message: 'Total marks should be greater than or equal to obtained marks', callback: function (value, validator) {
            ...
            }, ed_degree_obtained_marks: {
                validators: {
                    digits: { message: 'Only Digits Allowed' }, callback: {
                        message: 'Obtained marks should be less than or equal to total marks', callback: function (value, validator) {
                            var ob_marks = $('#ed_degree_obtained_marks').val();
                            var t_marks = $("#ed_degree_total_marks").val();
                            if (ob_marks === '' && t_marks === '') {
    ...
    }), $("#Specialities_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/specialities", data: $("#Specialities_form").serialize() + "&candidate_cnic=" + f + "&candidate_id=" + e, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        }, fields: { skill_id: { validators: { notEmpty: { message: "Please enter skill name" } } }, skill_level: { validators: { notEmpty: { message: "Please enter skill level" } } }, skill_summary: { validators: { notEmpty: { message: "Please enter skill summary" }, stringLength: { min: 5, max: 1e3, message: "The skill summary must be beteween 5 to 1000 characters" }, regexp: { regexp: /^[a-zA-Z0-9_@_#$,)(\d\-_.(),\s]+$/i, message: "Please enter valid summary" } } } }
    }), $("#edit_Specialities_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/edit_specialities", data: $("#edit_Specialities_form").serialize() + "&id=" + e, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
                        ...
                        $(".pre-loader2").hide();
                        b.data_id;
                        (b.status = "success") ? ($("#edit-specialities").modal("hide"),
                         $("#skills_table #" + e).html(""),
                         $("#loading_modal").modal("hide"),
                          $("#skills_table #" + e).append('<td class="" style="">' 
                            + g + '</td><td class="" style="">' 
                            + h + '</td><td class="" style="">' 
                            + i + '</td><td> <input type="hidden" value="' 
                            + f + '" id="edit_skill_id' + e + '" ><input type="hidden" value="' + g + '" id="edit_skill_name' + e + '" ><input type="hidden" value="' + h + '" id="edit_skill_level' + e + '" ><input type="hidden" value="' + i + '" id="edit_skill_summary' + e + '" ><a href="javascript:void(0)" class="skill_delete btn btn-success btn-sm" onclick="edit_skill(' + e + ')" data-tooltip="tooltip" title="Edit"><i class="fa fa-edit" aria-hidden="true"> </i> </a> <a href="javascript:void(0)" class="skill_delete btn btn-primary btn-sm" onclick="delete_skill(' + e + ')" data-tooltip="tooltip" title="delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>'), $(".notification").html("<div class='alert success_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Success! </strong>  Data updated successfully</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)) : ($("#edit-specialities").modal("hide"), $("#loading_modal").modal("hide"), $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong>  Data already exists! or Incompatible Data!</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300))
                    }, 1e3)
                }, error: function (a) {
                    $("#loading_modal").modal("hide"), $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong>  Error occured!</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)
                }
            ...
            ed_skill_level: { 
                validators: 
                { 
                    notEmpty: { message: "Please enter skill level" } 
                }   
            },
            ed_skill_summary: { 
                validators: 
                { 
                    notEmpty: { message: "Please enter skill summary" }, 
                    stringLength: { min: 5, max: 1e3, message: "The skill summary must be beteween 5 to 1000 characters" }
                    // regexp: { regexp: /^[a-zA-Z0-9_@_#$,)(\d\-_.(),\s]+$/i, message: "Please enter valid summary" } 
                } 
            } 
        }
    }), 
    ...
    $("#add_references_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/references", data: $("#add_references_form").serialize() + "&candidate_cnic=" + f + "&candidate_id=" + e, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/edit_references", data: $("#edit_references_form").serialize() + "&id=" + e, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/training", data: $("#add_training_form").serialize() + "&candidate_cnic=" + f + "&candidate_id=" + e, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
    ...
    }), $("#add_certification_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/certification", data: $("#add_certification_form").serialize() + "&candidate_cnic=" + f + "&candidate_id=" + e, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            ...
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/edit_certification", data: $("#edit_certification_form").serialize() + "&id=" + f, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        }, fields: { research_name: { validators: { notEmpty: { message: "Please enter research title" }, stringLength: { min: 1, max: 150, message: "The research title must be beteween 1 to 150 characters" }, regexp: { regexp: /^[a-zA-Z0-9 #@)(_,-.\s]+$/i, message: "Please enter alphanumerics only" } } }, publication_venue: { validators: { notEmpty: { message: "Please enter research venue" }, stringLength: { min: 1, max: 100, message: "The research venue must be beteween 1 to 100 characters" }, regexp: { regexp: /^[a-zA-Z0-9#@)(,-.\s]+$/i, message: "Please enter alphanumerics only" } } }, publication_link: { validators: { notEmpty: { message: "Please enter venue" }, uri: { message: "The publication link is not valid url" } } } }
    }), $("#edit_researchs_form").bootstrapValidator({
        excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
            var d = $("#url").val(), e = $("#ed_research_name").val(), f = $("#ed_publication_venue").val(), g = $("#ed_publication_link").val(), h = $("#ed_res_id").val();
            return $("#loading_modal").modal("show"), $.ajax({
                url: d + "new_profile_builder/edit_research", data: $("#edit_researchs_form").serialize() + "&id=" + h, type: "POST", success: function (a) {
                    setTimeout(function () {
                        var b = $.parseJSON(a);
                        if ("validation" == b.status)
        ...
        $("#add_achievement_form").bootstrapValidator({
            excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
                var d = $("#url").val(), e = $("#candidate_id").val(), f = $("#candidate_cnic").val(), g = $("#achievement_title").val(), h = $('#achievement_descriptions').val(), hh = tinyMCE.activeEditor.getContent({ format: 'text' });
                tinyMCE.triggerSave();
                return $("#loading_modal").modal("show"), $.ajax({
                    url: d + "new_profile_builder/achievement", data: $("#add_achievement_form").serialize() + "&fachievement_title=" + g + "&fachievement_descriptions=" + h + "&candidate_cnic=" + f + "&candidate_id=" + e, type: "POST", success: function (a) {
                        setTimeout(function () {
                            var b = $.parseJSON(a);
                            if ("validation" == b.status)
                                return $("#add_achievement_validation_erros").html(""), $(b.errors).each(function (a, b) {
                                    $("#add_achievement_validation_erros").append('<div class="alert-box error"><span>error: </span>' + b + "</div>")
                                }), $("#add-achievement").css("overflow-x", "auto"), $("#add-achievement").css("overflow-y", "auto"), $("#add-achievement").animate({ scrollTop: 0 }, 600), $("#loading_modal").modal("hide"), !1;
                            var c = b.data_id;
                            (b.status = "success") ? ($("#add-achievement").modal("hide"), $("#achievement_table tbody").append('<tr id="' + c + '"> <td>' + g + "</td><td>" + h + "</td>" + '<td><input type="hidden" value="' + g + '" id="edit_achievement_title' + c + '"><input type="hidden" value="' + h + '" id="edit_achievement_des' + c + '"><a href="javascript:void(0)" class="achievement_delete btn btn-success btn-sm" onclick="edit_ach(' + c + ')" data-tooltip="tooltip" title="Edit" > <i class="fa fa-edit" aria-hidden="true"></i></a> <a href="javascript:void(0)" class="certification_delete btn btn-primary btn-sm" onclick="del_ach(' + c + ')" data-tooltip="tooltip" title="delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td></tr>'), $("#loading_modal").modal("hide"), $("#add_achievement_form").bootstrapValidator("resetForm", !0), $(".notification").html("<div class='alert success_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Success! </strong>  Data inserted successfully</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)) : ($("#add-achievement").modal("hide"), $("#loading_modal").modal("hide"), $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong>  Data already exists! or Incompatible Data!</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300))
                        }, 1e3)
                    }, error: function (a) {
                        $("#loading_modal").modal("hide"), $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong>  Error occured!</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)
                    }
                }), !1
            }, fields: { achievement_title: { validators: { notEmpty: { message: "Please enter achievement title" }, stringLength: { min: 1, max: 50, message: "The achievement title must be beteween 1 to 50 characters" }, regexp: { regexp: /^[a-zA-Z0-9@#&.)(,_ -\s]+$/i, message: "Please enter alphanumerics only" } } }, achievement_descriptions: { validators: { notEmpty: { message: "Please enter achievement descriptions" }, stringLength: { min: 10, max: 200, message: "The achievement descriptions must be beteween 10 to 200 characters" }, regexp: { regexp: /^[a-zA-Z0-9@#&.)(,_ -\s]+$/i, message: "Please enter alphanumerics only" } } } }
        }),
        $("#edit_achievement_form").bootstrapValidator({
            excluded: [":disabled"], feedbackIcons: { valid: "glyphicon glyphicon-ok", invalid: "glyphicon glyphicon-remove", validating: "glyphicon glyphicon-refresh" }, submitHandler: function (a, b, c) {
                var d = $("#url").val(), f = ($("#candidate_cnic").val(), $("#ach_id").val()), g = $("#ed_achievement_title").val(), h = $('#ed_achievement_descriptions').val(), hh = tinyMCE.activeEditor.getContent({ format: 'text' });
                return $("#loading_modal").modal("show"), $.ajax({
                    url: d + "new_profile_builder/edit_achievement", data: $("#edit_achievement_form").serialize() + "&fachievement_title=" + g + "&fachievement_descriptions=" + h + "&id=" + f, type: "POST", success: function (a) {
                        setTimeout(function () {
                            var b = $.parseJSON(a);
                            if ("validation" == b.status)
                                return $("#edit_achievement_validation_erros").html(""), $(b.errors).each(function (a, b) {
                                    $("#edit_achievement_validation_erros").append('<div class="alert-box error"><span>error: </span>' + b + "</div>")
                                }), $("#edit-achievement").css("overflow-x", "auto"), $("#edit-achievement").css("overflow-y", "auto"), $("#edit-achievement").animate({ scrollTop: 0 }, 600), $("#loading_modal").modal("hide"), !1;
                            var c = b.data_id;
                            (b.status = "success") ? ($("#edit-achievement").modal("hide"), $("#loading_modal").modal("hide"), $("#achievement_table tbody tr#" + f).html(""), $("#achievement_table tbody tr#" + f).append('<td>' + g + "</td><td>" + h + "</td>" + '<td><input type="hidden" value="' + g + '" id="edit_achievement_title' + c + '"><input type="hidden" value="' + h + '" id="edit_achievement_des' + c + '"><a href="javascript:void(0)" class="achievement_delete btn btn-success btn-sm" onclick="edit_ach(' + c + ')" data-tooltip="tooltip" title="Edit" > <i class="fa fa-edit" aria-hidden="true"></i></a> <a href="javascript:void(0)" class="certification_delete btn btn-primary btn-sm" onclick="del_ach(' + c + ')" data-tooltip="tooltip" title="delete"><i class="fa fa-trash-o" aria-hidden="true"></i></a></td>'), $(".notification").html("<div class='alert success_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Success! </strong>  Data updated successfully</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)) : ($("#edit-achievement").modal("hide"))
                        }, 1e3)
                    }, error: function (a) {
                        $("#loading_modal").modal("hide"), $(".notification").html("<div class='alert danger_alert'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times; </a> <strong> Failure! </strong>  Error occured!</div>").fadeIn(300), $(".notification").delay(3200).fadeOut(300)
                    }
                }), !1
            }, fields: { ed_achievement_title: { validators: { notEmpty: { message: "Please enter achievement title" }, stringLength: { min: 1, max: 50, message: "The achievement title must be beteween 1 to 50 characters" }, regexp: { regexp: /^[a-zA-Z0-9@#&.)(,_ -\s]+$/i, message: "Please enter alphanumerics only" } } }, ed_achievement_descriptions: { validators: { notEmpty: { message: "Please enter achievement descriptions" }, stringLength: { min: 10, max: 200, message: "The achievement descriptions must be beteween 10 to 200 characters" }, regexp: { regexp: /^[a-zA-Z0-9@#&.)(,_ -\s]+$/i, message: "Please enter alphanumerics only" } } } }
        })
}),
    $(function () {
    ...
    $(document).ready(function (a) {
        ...
        $("#country_id").on("change", function (a) {
            ...
            return e.type.match("doc|pdf|msword|docx") ? f < 3 ? (c(this), void ("" != $("#fileToUpload").val() && $.ajax({
                url: b + "new_profile_builder/upload_resume", type: "POST", data: new FormData(this), contentType: !1, cache: !1, processData: !1, success: function (a) {
                    if ("failed" == a)
            ...
            $("#add-emp-job-information").on("shown.bs.modal", function () {
                ...
                $("#add_employee_job_information").bootstrapValidator("resetField", "desig_start_date"),
                    $("#add_employee_job_information").bootstrapValidator("resetField", "desig_end_date"), $("html").css("overflow-x", "hidden"), $("html").css("overflow-y", "hidden")
            }),
            $("#add-emp-job-information").on("hidden.bs.modal", function () {
                $('.ss').select2();
                $("#add_employee_job_information").trigger('reset'), $('#job_level').val(null).trigger('change'), $('#job_country').val(null).trigger('change'),
                    $("#add_employee_job_information").bootstrapValidator("resetField", 'job_level'), $("#add_employee_job_information").bootstrapValidator("resetField", 'job_country'), $("#add_job_validation_erros").html(""), $("html").css("overflow-x", "auto"), $("html").css("overflow-y", "auto")
            }),
            ...
            $("#desig_start_date_1").on("click", function () {
                ...
                1 == $("#ed_present").val() ? ($("#ed_present").removeAttr("Checked"), $("#ed_present").val(0), $("#ed_work_attach_document").attr("disabled", !1)) : ($("#ed_present").attr("Checked", "Checked"), $("#ed_present").val(1), $("#ed_work_attach_document").attr("disabled", !0)), document.getElementById("ed_present").checked ? ($("#ed_desig_end_date").val(""), $("#ed_work_attach_document").val(""),
                 $("#edit_employee_job_information").bootstrapValidator("resetField", "ed_desig_end_date"), $("#edit_employee_job_information").bootstrapValidator("resetField", "ed_work_attach_document"), $("#ed_desig_end_date").attr("disabled", !0), $("#ed_work_attach_document").attr("disabled", !0)) : ($("#edit_employee_job_information").bootstrapValidator("resetField", "ed_desig_end_date"), $("#edit_employee_job_information").bootstrapValidator("resetField", "ed_work_attach_document"), $("#ed_desig_end_date").attr("disabled", !1), $("#ed_work_attach_document").attr("disabled", !1))
            }),
        ...
        $("#is_govt_son_daughter_checkbox").on("click", function () {
            if ($("#is_govt_son_daughter_checkbox").attr('checked') == 'checked') {
                $("#is_govt_son_daughter_checkbox").val(1);
            } else {
                $("#is_govt_son_daughter_checkbox").val(0);
            }
        });
        if ($('#is_govt_son_daughter_checkbox').val() == 1) {
            $("#is_govt_son_daughter_checkbox").prop('checked', true);
        } else if ($('#is_govt_son_daughter_checkbox').val() == 0) {
            $("#is_govt_son_daughter_checkbox").prop('checked', false);
        }
        ...
        //    tinymce.init({
        //        selector: '#achievement_descriptions',
        //        height: 250,
        ...
        //        content_css: [
        //            '<?php echo base_url();?>assets/recruitment/tinymce/codepen.min.css'
        //        ],
        ...
        //                    return;     // Do nothing
        //                }
        //                $('#add_achievement_form').bootstrapValidator('revalidateField', 'achievement_descriptions');
        //            });
        //        }
        //    });
        $("#hafiz_quran").on("change", function () {
            if ($('#hafiz_quran').val() == 1) {
                $("#hafiz_quran_attachment_div").show();
                $("#hafiz_quran_attachment").prop('disabled', false);
                $("#hafiz_quran_attachment").prop('required', false);
            } else if ($('#hafiz_quran').val() == 0) {
                $("#hafiz_quran_attachment_div").hide();
                $("#hafiz_quran_attachment").prop('disabled', true);
                $("#hafiz_quran_attachment").prop('required', true);
            }
        });
        if ($('#hafiz_quran').val() == 1) {
            $("#hafiz_quran_attachment_div").show();
